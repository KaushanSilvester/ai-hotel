...
function Payment() {
  ...
  // If no booking context, redirect home
  useEffect(() => {
  ...
  // Payment state
  const [payWhen,  setPayWhen]  = useState("now");   // "now" | "hotel"
  const [method,   setMethod]   = useState("stripe"); // "stripe" | "payhere"
  ...
  const handlePayNow = async () => {
    ...
    setStep("processing");
    try {
      // For Stripe: in production, call your backend /api/create-payment-intent/
      // then confirm with Stripe.js. Here we simulate the flow.
  ...
  // ── Proceed button from select screen ──────────────────────────────────────
  const handleProceed = () => {
    if (payWhen === "hotel") {
      handlePayAtHotel();
    } else {
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .back-btn {
          ...
          font-family: 'DM Sans', sans-serif; cursor: pointer; color: #374151;
        ...
        .pay-when-option.selected .pwo-title { color: #2563eb; }
        .pwo-icon {
        ...
        .method-tab-logo { height: 24px; object-fit: contain; }
        ...
        .form-input.error { border-color: #dc2626; background: #fff; }
        .form-error { font-size: 11.5px; color: #dc2626; margin-top: 2px; }

        /* Card number with icon */
        .card-input-wrap { position: relative; }
        .card-input-icon { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); display: flex; gap: 4px; }
        ...
        .summary-card {
          ...
          box-shadow: 0 2px 16px rgba(0,0,0,0.06); position: sticky; top: 20px;
        }
        ...
        .room-preview-img-placeholder { width: 64px; height: 64px; border-radius: 10px; background: #e5e7eb; display: flex; align-items: center; justify-content: center; font-size: 24px; flex-shrink: 0; }
        .room-preview-name { font-size: 14px; font-weight: 700; color: #111827; margin-bottom: 4px; }
        .room-preview-meta { font-size: 12.5px; color: #6b7280; display: flex; flex-direction: column; gap: 2px; }
        .room-preview-meta span { display: flex; align-items: center; gap: 5px; }

        .summary-divider { height: 1px; background: #f3f4f6; margin: 14px 0; }

        .summary-row { display: flex; justify-content: space-between; align-items: center; font-size: 13.5px; color: #6b7280; margin-bottom: 10px; }
        ...
        .success-circle {
          ...
          margin-bottom: 20px; box-shadow: 0 8px 30px rgba(22,163,74,0.3);
        ...
        .success-details {
          background: #f9fafb; border-radius: 14px; padding: 18px 24px;
          margin-bottom: 24px; width: 100%; text-align: left;
        }
        ...
        .go-dashboard-btn {
          padding: 13px 32px; background: #111827; color: #fff;
          border: none; border-radius: 12px; font-size: 15px; font-weight: 700;
          font-family: 'DM Sans', sans-serif; cursor: pointer;
          box-shadow: 0 4px 16px rgba(0,0,0,0.2); transition: background 0.15s;
        }
        ...
        .hotel-circle {
          ...
          display: flex; align-items: center; justify-content: center;
          margin-bottom: 20px; box-shadow: 0 8px 30px rgba(5,150,105,0.3);
          font-size: 36px;
          animation: popIn 0.4s cubic-bezier(0.34, 1.56, 0.64, 1);
        }
      ...
      <div className="pay-root">
        ...
        <div className="pay-layout">
          ...
          <div className="pay-left">
                ...
                <div className="success-screen">
                  ...
                  <div className="success-details">
                    <div className="success-detail-row">
                      <span className="success-detail-label">Room</span>
                      <span className="success-detail-val">{room.room_type}</span>
                    </div>
                    <div className="success-detail-row">
                      <span className="success-detail-label">Check-in</span>
                      <span className="success-detail-val">{checkIn}</span>
                    </div>
                    <div className="success-detail-row">
                      <span className="success-detail-label">Check-out</span>
                      <span className="success-detail-val">{checkOut}</span>
                    </div>
                    <div className="success-detail-row">
                      <span className="success-detail-label">Guests</span>
                      <span className="success-detail-val">{guests}</span>
                    </div>
                    <div className="success-detail-row">
                      <span className="success-detail-label">Amount Paid</span>
                      <span className="success-detail-val" style={{ color: "#16a34a" }}>Rs. {total.toLocaleString()}</span>
                    </div>
                    <div className="success-detail-row">
                      <span className="success-detail-label">Payment Via</span>
                      <span className="success-detail-val">{method === "stripe" ? "Stripe (Card)" : "PayHere"}</span>
                    </div>
                  </div>
            ...
            {step === "hotel_done" && (
              <div className="pay-card">
                <div className="success-screen">
                  ...
                  <div className="success-sub">
                    Your room is reserved. Payment will be collected at check-in.<br/>
                  ...
                  <div className="success-details">
                    ...
                    <div className="success-detail-row">
                      <span className="success-detail-label">Amount Due</span>
                      <span className="success-detail-val">Rs. {total.toLocaleString()} (at hotel)</span>
                    </div>
                  </div>
                  <button className="go-dashboard-btn" onClick={() => navigate("/dashboard")}>
                    View My Bookings
            ...
            {(step === "select" || step === "card") && (
              <div className="pay-card">
                <div className="section-title">
                  <span className="section-num">1</span> When would you like to pay?
                </div>
                <div className="pay-when-grid">
                  {/* Pay Now */}
                  <div className={`pay-when-option ${payWhen === "now" ? "selected" : ""}`} onClick={() => setPayWhen("now")}>
                    {payWhen === "now"
                      ? <div className="selected-tick"><CheckIcon /></div>
                      : null
                    }
                  ...
                  <div className={`pay-when-option ${payWhen === "hotel" ? "selected" : ""}`} onClick={() => setPayWhen("hotel")}>
                    ...
                    {payWhen === "hotel"
                      ? <div className="selected-tick" style={{ top: 36 }}><CheckIcon /></div>
                      : null
                    }
            ...
            {/* ── STEP 2: PAYMENT METHOD (only if Pay Now) ── */}
            {step === "select" && payWhen === "now" && (
              <div className="pay-card">
                <div className="section-title">
                  <span className="section-num">2</span> Choose payment method
                </div>
                <div className="method-tabs">
                  ...
                  <div className={`method-tab ${method === "stripe" ? "active" : ""}`} onClick={() => setMethod("stripe")}>
                    ...
                    <div>
                      <div className="method-tab-name">Credit / Debit Card</div>
                      <div className="method-tab-sub">Visa, Mastercard, Amex</div>
                    </div>
                  </div>
                  {/* PayHere */}
                  <div className={`method-tab ${method === "payhere" ? "active" : ""}`} onClick={() => setMethod("payhere")}>
                    ...
                    <div>
                      <div className="method-tab-name">PayHere</div>
                      <div className="method-tab-sub">Sri Lanka cards & wallets</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="pay-card">
                ...
                <div className="card-form">
                  ...
                  <div className="secure-badge">
                    <LockIcon /> Your payment is encrypted and secure
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="summary-card">
              ...
              <div className="room-preview">
                {room.image
                  ...
                  : <div className="room-preview-img-placeholder">🏨</div>
                }
                <div>
              ...
              <div className="summary-row">
                <span className="summary-row-label">Rs. {parseFloat(room.price).toLocaleString()} × {nights} night{nights > 1 ? "s" : ""}</span>
                <span>Rs. {subtotal.toLocaleString()}</span>
              </div>
              <div className="summary-row">
                <span className="summary-row-label">Taxes & fees (10%)</span>
              ...
              <div className="pay-guarantee">
                🔒 &nbsp;
                <span>
                  <strong>Best price guarantee.</strong> Free cancellation if you choose Pay at Hotel.
